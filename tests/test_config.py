...
def test_yaml_parser_valid(mocker):
    ...
    mock_open = mocker.patch(
        'builtins.open', mocker.mock_open(read_data=yaml.dump(test_data))
    )
...
def test_yaml_parser_yaml_error_with_mark(mocker):
    ...
    mark = yaml.Mark(
        'fake/path/test.yaml', 1, 5, pointer=10, column=5, buffer='context'
    )
    mock_safe_load.side_effect = yaml.MarkedYAMLError(
        'error', mark, 'context', 'problem', 'context_mark'
    )
...
def test_yaml_parser_yaml_error_without_mark(mocker):
    ...
    mock_safe_load = mocker.patch('yaml.safe_load')
    mock_safe_load.side_effect = yaml.MarkedYAMLError(
        'error', None, 'context', 'problem', 'context_mark'
    )
...
def test_yaml_parser_generic_exception(mocker):
    ...
    mock_safe_load = mocker.patch(
        'yaml.safe_load', side_effect=yaml.YAMLError('Generic error')
    )
